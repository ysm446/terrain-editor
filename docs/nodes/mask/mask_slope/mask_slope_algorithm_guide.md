...
各セルで左右と上下の中央差分を取り、地形サイズからセル間隔を求めます。
...
`Slope Min` と `Slope Max` の間は smoothstep で滑らかに遷移します。
差を狭くすると硬い境界になり、広くするとなだらかな境界になります。

## 使い分け

- 急斜面マスク: `Invert` off。
- 平地マスク: `Invert` on。
- 岩肌抽出: `Slope Min` を 35-45 度、`Slope Max` を 55-70 度。
- 雪や草の制限: 平地マスクとして使い、後段の `Mask Blend` で乗算する。
