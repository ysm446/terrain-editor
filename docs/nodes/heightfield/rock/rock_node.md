...
| 設定 | 既定値 | 役割 |
| --- | --- | --- |
| `Rock Style` | Polygonal | 岩の基本シェープ。`Classic` は従来互換、`Polygonal` は上面を平面にカットした低ポリゴン状、`Shard` は細長い破片状 |
| `Orientation Rule` | Flat | 地形勾配に対する岩の扱い。`Flat` は従来通り、`Follow Ground` は斜面距離と法線の上向き成分を使う、`Slope Oriented` は岩の向きを斜面方向へ寄せる |
| `Layer Count` | 1 | 別シードの岩散布レイヤー数。増やすほど密度と不規則さが増える |
| `Seed` | 0 | ハッシュオフセット |
...
| `Edge Sharpness (%)` | 100 | シルエット形状。0=円形ドーム、>0 で多角形(4–7 角)で完全クリップ。`Polygonal` / `Shard` では最低 65% 相当の多角形シェープを保つ |
...
3. **走査範囲**: `searchRadius = max(1, ceil(rockSizeMaxCells × 0.5 × pow(2, aspectVar + styleBoost) - 0.05))`。最大ドーム半径とアスペクト最大伸長を考慮した最悪ケース。`Shard` では細長い破片を拾うため `styleBoost = 0.65` を足す。既定値で 3×3、巨石+細長設定で最大 ~7×7。
4. **入力地形の勾配**: `Orientation Rule` が `Flat` 以外のとき、入力 Heightmap の中央差分から `gradX / gradZ`、斜面方向、法線の上向き成分を求める。
5. **各ピクセルで Layer Count 分の近傍セルを走査**:
   ...
   - **岩ごとのパラメータをハッシュから決定**:
      ...
      - 回転角 `θ = (hash - 0.5) × 2π × rotationVariation`。`Slope Oriented` では斜面方向角にこのランダム回転を足す。
      - アスペクト `aspect = pow(2, aspectVariation × (2×hash − 1) + styleBoost)`(対称な乗法レンジ、面積保存)。アスペクト軸 `aspect_x` ∈ {`aspect`, `1/aspect`} を別ハッシュで選び、`aspect_z = 1 / aspect_x`
   - **ローカル座標変換**: 回転 → アスペクト除算で楕円距離 `d_local` を計算。`Follow Ground` / `Slope Oriented` では勾配方向の高さ差も距離へ加え、斜面上の距離に近づける。`d_local ≥ domeRadius_per` ならスキップ。
   - **ドーム高さ(多角形ハードクリップ + 内部ブレンド)**:
     - 円形成分 `radialT = 1 - d_local / domeRadius_per`
      - `edgeSharpness > 0` のときは岩ごとに凸多角形 SDF を計算。`Classic` は 4–7 辺、`Polygonal` は 5–8 辺、`Shard` は 4–6 辺。各辺の法線角は `(2π/N) × i + jitter`、辺の中心からの距離(inradius)は `domeRadius_per × cos(π/N) × (1 - hash × jitter)`。**多角形の外側のピクセルは即 continue でこの岩の寄与なし(ハードクリップ)**。
      - 内側成分 `polyhedralT`: `Classic` は SDF の内部距離を `baseInradius` で正規化する。`Polygonal` / `Shard` は岩ごとにオフセンター頂点を置き、各辺から頂点までの距離で正規化するため、中心固定のドームではなく低ポリゴンの面が出やすい。`Polygonal` は一定以上の高さを平面に切り落とし、上面では表面ディテールを弱める。
...
| 目的 | パラメータの方向性 |
| --- | --- |
| GeoGen 風の散らばった岩塊カバー(参考画像) | `Rock Style` Polygonal / `Density` 4-6m / `Rock Size` 3-12m / `Rotation` 1.0 / `Aspect` 0.4-0.6 / `Edge Sharpness` 0.8-1.0 / `Facet Sharpness` 0.6+ |
| 細かい多面体岩肌(垂直崖) | `Rock Style` Polygonal / `Density` 2-4m / `Rock Size` 2-5m / `Aspect` 0.2 / `Edge Sharpness` 0.9 / `Facet Sharpness` 0.7+ / `Facet Scale` 3-4 |
| 巨石を密集 | `Rock Style` Polygonal / `Density` 6-10m / `Rock Size` 12-30m / `Aspect` 0.3-0.5 / `Rock Height` 3-8m |
| 散らばった岩 | `Rock Style` Classic または Polygonal / `Density` 15m+ / `Coverage` 0.3-0.5 / `Rock Size` 6-10m |
| 結晶質クリスタル風 | `Rock Style` Shard / `Density` 6m / `Rock Size` 5-7m / `Aspect` 0.3-0.5 / `Facet Sharpness` 1.0 / `Facet Scale` 1.5-2.5 / `Bumpiness` 0.8+ |
| 斜面に沿った岩肌 | `Orientation Rule` Follow Ground / `Layer Count` 2-3 / `Density` 3-6m / `Rock Size` 4-10m |
| 斜面方向に流れる岩片 | `Orientation Rule` Slope Oriented / `Rock Style` Shard / `Aspect` 0.5+ / `Layer Count` 2 |
...
- 岩の大きさは `Rock Size Min/Max` で直接指定し、`Density` は中心のばらまき間隔として残しています。GeoGen の `Grid size` のように配置幅だけで岩サイズを決める方式より、密度と岩直径を独立して調整できます。
