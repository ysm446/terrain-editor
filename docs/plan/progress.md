...
- ビューポートリセットのショートカットを `A` キーへ移し、初期カメラ距離は地形全体と背景が入りやすいよう少し遠めにした。
- `Reset View` / `A` キーの距離計算は現在の FOV を使い、リセット時も FOV を維持する。
- 初期カメラ角は yaw -30 度、pitch +30 度にし、`O` キーでカメラ自動回転を切り替えられる。
- `D` キーで Camera タブの Depth of Field を切り替えられる。
- `C` キーでボリューム雲の描画オン / オフを切り替えられる。
- `F12` のスクリーンショットはバックバッファ読み戻し方式で、コンテンツ部分だけをクライアント解像度そのままで保存する。保存後は右下の通知をクリックすると保存先フォルダーを開ける。
- `表示 > ウィンドウサイズ` でコンテンツ部分を 1280x720 - 3840x2160 のプリセットに合わせられる。
- `表示 > ヘルプ` でショートカット一覧をアプリ内に表示できる。一覧のドキュメントは `docs/ui/shortcuts.md`。
- インスペクタに `環境設定` タブを追加し、言語設定と `FPS` / `FPS Limit` を移動した。
- `環境設定` の `FPS` 表示と `FPS Limit` はアプリ設定専用にし、プロジェクトファイルへ保存しないようにした。
- `Viewport Mesh`、`LOD`、`Mesh Backend`、`Tessellation`、地形/雲の品質系設定、新規ノードの Backend 既定値を `環境設定` へ移し、アプリ設定として保存するようにした。ノード個別 Backend は従来どおりノード側に保存する。
- デバッグ用途の `Surface` 表示切り替えは `設定` から外し、`診断` タブの `Viewport Debug` へ移動した。
- `環境設定` タブはインスペクタの一番右に配置した。
- インスペクタはタブ本文ごとにスクロールするようにし、`カメラ` や `環境設定` の長い内容でもタブ列や右ペイン全体ではなくタブ内だけにスクロールバーが出る。
- インスペクタのタブ内スクロールバーは、親領域で見切れない幅に収めつつ、各タブで同じ右余白の位置に揃えて表示する。
...
- ビューポートは、カメラ初期化、`A` キーリセット、Depth of Field、GPU Displacement / Tessellation、地形境界表示、近い地形でのマスク表示、水面表示に対応済み。
...
- Camera タブの `Color Temperature (K)` は HDR / SDR 共通の色温度設定として扱う。低い値で夕焼け寄りの暖色、高い値で寒色へ寄せられる。
- `HDR Viewport` は既定でオフ。オンにすると 16-bit float HDR 経路を使い、オフでは HDR 以前の 8-bit ビューポート経路に戻してトーンマップと露出調整を使わず表示する。
- `HDR Viewport` と露出設定は Camera タブの HDR 設定として扱う。HDR オフ時も露出項目は非アクティブ状態で表示する。
...
- Path 編集モード中は Camera の被写界深度フォーカスピックへ入らないようにし、被写界深度フォーカスピックのカーソル表示と `F+クリック` は `Depth of Field` が有効なときだけ発動するようにした。
...
- `Camera` の `Depth of Field` で、3D ビューポート上の地形を `F+クリック` してフォーカス距離を設定できるようにした。`F` 押下中は十字カーソルと地形上のターゲットを表示し、Camera タブの `フォーカスを選択` ボタンからも同じピック操作を開始できる。
