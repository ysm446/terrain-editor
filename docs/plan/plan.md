...
- `Ctrl+クリック` による DOF フォーカス距離ピックは便利だが、将来の Spline / River / 配置系などのビューポート編集モードと競合する可能性がある。ビューポート操作モードを整理するときに、Camera モード時だけ有効にする、またはツール選択中だけ有効にする形へ寄せるか検討する。
...
- ノードテクスチャーのエクスポートでは、将来、エクスポート対象ノードをリスト管理して一括出力できる仕組みを検討する。
