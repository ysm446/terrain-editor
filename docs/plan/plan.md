...
- グローバルな `Terrain Size (m)` は初期実装済み。今後は Import Heightmap の配置オフセット、Fill Height、複数ハイトマップ合成が必要かを検討する。
...
- `Terrain Size (m)` は 512 / 1024 / 2048 / 4096m から選ぶグローバルな地形キャンバスサイズとして扱う。
- `Import Heightmap` の `Scale (m)` は読み込んだ画像がグローバル地形内で占める実サイズ。グローバルサイズを超える場合は中央でクロップし、小さい場合は外側を高さ 0 にする。
- ハイトマップ入力を持たないマスクノードのプレビューも、グローバルな `Terrain Size` に合わせる。
