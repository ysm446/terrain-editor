...
- `グレースケール + 斜線` モード限定で、最終的な斜線色にハーフランバート `lerp(0.5, 1.0, (n·L * 0.5 + 0.5)²)` を乗算するようにしました。GeoGen のリファレンス画像のように、太陽方向に向いた斜面 (=1.0) は素のハッチが明るく見え、横向き (=0.625)・反対側 (=0.5) は徐々に暗い陰影にハッチが沈む形になります。床は 0.5 のまま二乗を効かせることで、横〜背面側の陰影コントラストを上げつつ反対側が真っ黒に潰れないようにしています。中間域 (連続マスク) も同じハーフランバートで陰影が乗ります。あわせて `mask <= 0.01` 側 (黒の所) のハッチ縞色をグレー (0.5) → 白 (1.0) に変更しました。乗算で白縞が halfL の階調そのまま見えるようになり、暗部でも縞が沈み込まずに陰影パターンとして読めます。`mask >= 0.99` 側 (白の所) は従来どおり 白×3 + グレー×1。`グレースケール` / `グレー×オレンジ` モードと壁面 sentinel 経路には影響しません。
