...
- UI の実装ルールを [docs/design/style-guide.md](design/style-guide.md) に整備しました。これまで方針だけを書いた雛形でしたが、実装から抽出した具体的なルールで中身を埋めています。ローカライズの線引き (ノードのパラメータ名は英語のまま、説明はツールチップに日本語を用意、翻訳するヘッダー/タブには `###` の安定 ID を付ける)、画面レイアウトの構造とスプリッターの仕様、プロパティ行の型 (2 列テーブル、ラベル列幅 112 / 184 / 200 / 210、スライダー・入力欄・コンボ・パス行などの固定幅、`↺` の既定値リセットボタンが「既定値から変更されているか」の唯一の視覚表現であること、ツールチップの書き方)、変更通知と Undo の作法 (`MarkGraphChanged` の理由文字列、ドラッグ 1 回で Undo 1 段)、テーマ色の引き方とアプリ独自色の一覧、ノードエディターの寸法・カテゴリ色・ピンの型色・評価パルス、ビューポートオーバーレイ、トースト / ステータスバー / デバッグログの使い分けを収録し、最後に新しい UI を追加するときのチェックリストを付けています。

- UI 操作のドキュメントを追加しました。[docs/ui/shortcuts.md](ui/shortcuts.md) にキー / マウス操作の一覧 (アプリ内の `表示 > ヘルプ` と同じ内容 + 補足)、[docs/ui/screenshot.md](ui/screenshot.md) にスクリーンショットの保存先・撮影方式 (バックバッファ読み戻し)・ウィンドウサイズプリセット・通知トーストの仕様をまとめています。あわせて README のドキュメント索引に、これまで未掲載だったカメラ / 被写界深度 / 水面 / アンビエントオクルージョン / マスクテクスチャー描画へのリンクを追加し、`docs/plan/progress.md` の進捗サマリーを更新しました。

- `表示 > ヘルプ` を追加しました。オンにするとショートカット一覧のウィンドウが開き、「ファイル / 編集」「3Dビュー」「2Dビュー」「ノードネットワーク」「パス編集」のカテゴリごとにキー操作とマウス操作を表示します (キー表記も UI 言語に合わせて英語 / 日本語を切り替えます)。ウィンドウは移動・リサイズでき、位置とサイズは `imgui.ini`、表示のオン/オフは `data/app_settings.json` に保存されます。タイトルバーの × でも閉じられます。

- F12 のスクリーンショットを、ウィンドウ枠・タイトルバーを含まない「コンテンツ部分だけ」の保存に変更しました。画面 (デスクトップ) からの切り出しをやめ、スワップチェーンのバックバッファを直接読み戻して PNG 化します。これにより、(1) タイトルバー・ウィンドウ枠・影が入らない、(2) 手前に別のウィンドウが重なっていても、また画面外へはみ出していても正しく撮れる、(3) ディスプレイの拡大率 (DPI スケーリング) に影響されず、常にクライアント領域と同じ解像度 (例: 1920x1080 なら 1920x1080 ピクセル) で保存される、という挙動になります。撮影を要求したフレームは通知トーストを描画しないため、直前の通知が写り込むこともありません。`src/screenshot_capture.*` は画面キャプチャ (`CaptureWindowScreenshot`) を廃止し、RGBA ピクセル列を PNG に保存する `SaveRgbaScreenshot` に置き換えました。

- 画面右下に通知トーストを表示するようにしました。スクリーンショットを保存すると「保存しました (幅 x 高さ)」とファイル名が右下に出て、クリックすると Explorer で保存先フォルダーをファイル選択状態で開きます。右クリックまたは約 6 秒の経過でフェードアウトして消え、ホバー中は消えません (複数同時に出た場合は下から積み上げ、最大 4 件)。

- `表示 > ウィンドウサイズ` にクライアント領域 (コンテンツ部分) のサイズプリセット (1280x720 / 1600x900 / 1920x1080 / 2560x1440 / 3840x2160) を追加しました。現在のコンテンツサイズも同メニューに表示します。スクリーンショットはコンテンツ部分と同じ解像度で保存されるため、例えば 1920x1080 を選べばそのまま 1920x1080 の PNG が得られます (ウィンドウ枠の分だけ画面からはみ出しても、撮影結果には影響しません)。

- 被覆型の表土ノード `Soil` を追加しました。`Snow` と共有の「注入 → 安息角による再配分 → 表面平滑化」コア (CPU / GPU 両対応、GPU は `snow_compute.hlsl` を共有) を土向け既定値 (安息角 32°、控えめな表面平滑化) で実行し、緩斜面・上面に表土をかぶせて急斜面・崖では剥がして岩盤を露出させます。`Sediment` (谷埋め型: 岩屑が溝・谷底に溜まる) では出せない「土が岩の上面にかぶさる」表現を担当し、これまで `Snow` を土の堆積に代用していた用途を置き換えます。`Soil` 固有のパラメータとして、基盤の傾斜が安息角に近いほど注入を減らす `Slope-Dependent Emission (%)` (尾根・崖の岩盤露出が早く出る) と、`Mask Mode` (Coverage = Snow と同じほぼ白黒の被覆マスク / Thickness = Sediment と同じ厚み正規化マスク) を追加しています。実装にあたり Snow の再配分コアを共有関数 (`GranularSettle`) へ抽出しましたが、`Snow` ノード自体の挙動は変わりません。

- `Droplet Erosion` ノードに GPU バックエンド (D3D12 compute) を追加しました。プロパティの `Backend` で CPU / GPU を切り替えられます (既定 GPU、シェーダー / ディスパッチ失敗時は CPU に自動フォールバック)。CPU 版は「水滴を 1 粒ずつ逐次に流し、後続の水滴が直前までの削り跡を見る」逐次アルゴリズムでそのまま並列化できないため、GPU 版は Fluvial Erosion と同じスナップショット方式に置き換えています。反復ごとに全水滴を 1 スレッド = 1 粒子で並列トレースし (凍結した高さに対して読み取り)、削り・堆積を固定小数点 (1/4096 m) アトミックで per-cell に集積し、ソフト飽和付きの合計で反映、次の反復で刻まれた水路が帰還する、という流れです。各レベルの水滴数を複数反復に分割して帰還を確保し、マルチグリッドのレベル間アップサンプルも GPU 上で行います。加算順序に依存しないため決定的で、CPU 版とは視覚的に同等ですがビット単位では一致しません (容量モデル・慣性・蒸発・境界フェードなどパラメータの意味は CPU 版と共通)。なお放射ブラシ (Erosion Radius) は GPU では per-step のアトミック数が膨大になるため、バイリニア 4 セルスプラット + 反復あたりのソフト飽和上限で代替しています。

- `Droplet Erosion` の境界フェード幅をメートル基準にし、地形端に残っていた針状の深い切れ込みを解消しました。フェード幅がセル単位 (端から 3 セル) だったため、高解像度では物理的に薄くなりすぎ、主要な水路が端へ抜ける所でフル深度のまま急に削りが止まって針状の谷が残っていました。固定の物理幅 (24 m) でなだらかに削りを 0 へ落とすことで、どの解像度でも端の切れ込みが滑らかになります。

- `Droplet Erosion` で地形端の排水口が深く掘られ、メッシュのスカートが前端から下方へ垂れ下がる(縦の幕状に突き抜ける)アーティファクトを修正しました。すべての水滴が地形端で経路を終えるため、フェードが無いと端の排出セルが毎パス削られて深い切れ込みになっていました(水滴が集中する高解像度ほど顕著)。Fluvial Erosion と同じく、境界から数セルの範囲で削り・過飽和堆積をフェードアウトするようにしました(窪地埋めの単セル堆積はそのまま)。

- `Droplet Erosion` がシミュレーション解像度を変えても結果が一貫するようにしました。物理量で持つべきパラメータがセル(ステップ)単位で書かれていたため、解像度を変えると水路の到達距離・幅・密度がずれていました。Fluvial Erosion と同じくメートル/セル密度基準に統一しています。(1) `Max Lifetime` (ステップ数) を `Travel Distance (m)` (メートル) に変更し、セルサイズでステップ数に換算するようにしました。解像度を上げて 1 ステップの物理距離が縮んでも到達距離が一定になります。(2) `Evaporation` を「1 ステップあたり」から「1 メートルあたり」に変更し、セルサイズで複利圧縮するようにしました。細かいグリッドほどステップ数が増えて水が早く枯れる問題を解消しています。(3) `Erosion Radius` をセルからメートルに変更し、水路・土手の幅が解像度によらず一定になるようにしました。(4) `Particle Count` (絶対数) を `Droplet Density` (1 セルあたりの水滴数) に変更しました。絶対数のままだと密度 = 数 / セル数 が解像度で変化し、`ParticlesForLevel` の配分を通じて粗いレベルの粒子数まで変わって大局構造がずれていました。割合で持つことで全レベル・全解像度で密度が一定になります。容量モデルの勾配項 (`slope = -dH / cellSize`) と速度更新は元から解像度不変です。旧 `.terrainproj` のこれら 4 パラメータは新しい既定値で読み込まれます。

- `Droplet Erosion` のドキュメントを独立フォルダ (`docs/nodes/heightfield/droplet_erosion/`) に分離しました。これまで `Fluvial Erosion` と 1 ファイルを共有していたノード解説を `droplet_erosion_node.md` として独立させ、新規のアルゴリズム入門 `droplet_erosion_algorithm_guide.md` (水滴の状態・1 ステップの「削る/運ぶ/置く」判断・運搬容量・放射ブラシ・マルチグリッド・Fluvial Erosion との違い・チューニング指針) を追加しました。`fluvial_erosion_node.md` は `Fluvial Erosion` 専用に整理し、ノードドキュメント索引 (`docs/nodes/README.md`) のリンクを更新しました。

- `Droplet Erosion` の「水路が中腹で途切れる」「堆積が瘤状に盛り上がる」問題を修正しました。(1) `Max Lifetime` のデフォルトを 64→256、`Evaporation` を 0.02→0.005 に変更し、水滴が尾根から谷底まで届くようにしました (旧設定では 64 セルで必ず消滅し、容量 ∝ 水量のため寿命末期に毎回まとめて荷下ろししていました)。(2) 容量超過時の堆積を浸食と同じ放射ブラシで撒くようにし、1 セルに点置きされた堆積が粗いピラミッドレベルのアップサンプルで丸い瘤になる問題を解消しました (窪地埋めの堆積は意図的に単セルのまま)。(3) 運搬容量の勾配項をセルサイズで正規化し、粗いレベルだけ容量・浸食が数倍効く解像度非一貫性を解消しました。

- `Fluvial Erosion` ノードに GPU バックエンド (D3D12 compute) を追加しました。プロパティの `Backend` で CPU / GPU を切り替えられます (既定 GPU、シェーダー / ディスパッチ失敗時は CPU に自動フォールバック)。アルゴリズムは CPU 版と同一の粒子輸送で、反復ごとに全粒子を 1 スレッド = 1 粒子で並列トレースし、スプラットを固定小数点 (1/4096 m) アトミックで集積します。加算順序に依存しないため GPU 版の結果は完全決定的です (CPU 版と視覚的に同等、ビット単位では不一致)。マルチグリッドのレベル間アップサンプルも GPU 上で行い、転送は最初のアップロードと最後のリードバックのみです。

- `Fluvial Erosion` の削りすぎを抑えました。粒子が集中するセル(谷底)の 1 反復あたりの高さ変化上限を引き下げ(セルサイズ × 0.25 → 0.12)、谷が元の起伏に対して過剰に深掘りされないようにしました。`Erosion Strength` のデフォルトも 1.0 (レンジ最大) → 0.5 に変更し、強める余地を持たせています。また、地形端で全粒子の経路が収束・終端することで境界セルが毎反復最大まで削られ、針状の切れ込みが並ぶアーティファクトが出ていたため、境界から数セルの範囲で削り・堆積をフェードアウトするようにしました。

- `Fluvial Erosion` の水路が山頂付近で途切れず谷底まで連続するように改善しました。削り量の基準を「地形勾配と粒子の運動量(慣性速度を等価勾配に換算した値)の大きい方」に変更し、斜面が緩む中腹〜山麓でも勢いのある流れが削り続けるようにしました。また窪地に捕まった粒子が往復しながら削り続けて点状の穴(ポックマーク)を作っていたため、降下方向に動いている間だけ削るように修正し、運動量が尽きた粒子は土砂を堆積して窪地を自己修復するようにしました。デフォルト値も `Channel Length` 128m→512m (粒子が尾根から谷底まで届く)、`Max Erosion Angle` 30°→45° (急な山腹で水路が分断されない) に変更し、`Channel Length` スライダーの上限を 1024m に拡大しました。

- `Fluvial Erosion` ノードの浸食カーネルを刷新し、滑らかな斜面にも樹枝状の水路が刻まれるようにしました。従来の「進行方向の前後平均へ高さを寄せる」方式は平面的な斜面では変化量がほぼゼロになり、前後セル範囲へのクランプにより周囲より深く掘り下げることが構造的にできませんでした。新カーネルでは粒子が局所勾配に比例して地形を削り (stream power 方式)、削った土砂を運搬して `Deposit Angle` 未満の緩斜面に堆積させます。セルごとの高さ変化は「平均」ではなく「ソフト飽和付きの合計」で適用するため、粒子が集中するセルほど速く深くなり (流量→掘り込み→流量の正のフィードバック)、1 反復あたりの変化上限によりスパイクは防止されます。デフォルト値も見直し、`Wear Angle` 15°→3° (緩地形でも浸食ゲートが開く)、`Deposit Angle` 0°→2° (堆積が実際に発生する)、`Flow Volume` 0→0.35 (wear フィードバックによる水路の自己強化を標準で有効化)、`Feature Size` 8m→16m (マルチグリッドがより粗いレベルから谷を形成) に変更しました。
...
## 0.19.123 - 2026-05-27 05:51

- インスペクタのタブ内スクロール領域に共通の右余白を持たせ、`水面`、`カメラ`、`環境設定` などでスクロールバーの位置が揃うようにしました。

## 0.19.122 - 2026-05-27 05:47

- `カメラ` / `環境設定` タブ内スクロールバーが親領域の右端で見切れないよう、スクロール領域の幅を調整しました。

## 0.19.121 - 2026-05-27 05:43

- インスペクタのタブ内スクロールバーが他のスクロール領域と同じ右端位置に揃うよう調整しました。

## 0.19.120 - 2026-05-27 05:40

- インスペクタの各タブ本文を個別のスクロール領域にし、`カメラ` や `環境設定` でスクロールバーが右ペイン全体ではなくタブ内に出るようにしました。

## 0.19.119 - 2026-05-27 05:32

- インスペクタの `環境設定` タブをタブ列の一番右へ移動しました。

## 0.19.118 - 2026-05-27 05:28

- `設定` タブからデバッグ用途の `Surface` 表示切り替えを外し、`診断` タブの `Viewport Debug` に移動しました。

## 0.19.117 - 2026-05-27 05:20

- `Viewport Mesh`、`LOD`、`Mesh Backend`、`Tessellation`、地形シャドウ解像度、雲の品質/影サンプル数を `環境設定` へ移動し、アプリ設定専用として保存するようにしました。プロジェクトファイルにはこれらの表示負荷・実行環境依存の値を保存しません。
- `環境設定` に新規ノードの Backend 既定値を追加しました。新しく作るノードはこの既定値を使い、保存済みノードやコピーしたノードは各ノード個別の Backend を維持します。

## 0.19.116 - 2026-05-27 05:05

- `環境設定` に移した `FPS` 表示と `FPS Limit` をアプリ設定専用にし、プロジェクトファイルへ保存しないようにしました。プロジェクトを開いてもアプリ側の表示/フレーム制限設定が維持されます。

## 0.19.115 - 2026-05-27 05:03

- インスペクタに `環境設定` タブを追加し、言語設定と `FPS` / `FPS Limit` を移動しました。表示設定タブはビューポート表示内容の設定に絞りました。

## 0.19.114 - 2026-05-27 04:58

- 被写界深度のフォーカス距離ピックを `Ctrl+クリック` から `F+クリック` へ変更しました。`F` キーを押している間にターゲットを表示し、クリックした地形位置をフォーカス距離にします。

## 0.19.113 - 2026-05-27 04:55

- `HDR Viewport` の既定値をオフにし、新規プロジェクトや初期設定では従来の 8-bit ビューポート経路から始まるようにしました。

## 0.19.112 - 2026-05-27 04:53

- `Reset View` / `A` キーの距離計算で現在の FOV を使うようにしました。リセット時も FOV は維持し、広角では近め、望遠では遠めにしてメッシュ全体が画面に収まりやすくなります。

## 0.19.111 - 2026-05-27 04:52

- `D` キーで Camera タブの `Depth of Field` をオン / オフできるようにしました。

## 0.19.110 - 2026-05-27 04:51

- `Reset View` / `A` キーで戻る初期カメラ角を yaw -30 度、pitch +30 度にし、X- 側から地形を見る向きに変更しました。
- `O` キーで Camera タブの `カメラを回す` をオン / オフできるようにしました。

## 0.19.109 - 2026-05-27 04:47

- ビューポートリセットのショートカットを `F` キーから `A` キーへ変更しました。`Reset View` ボタンと同じ処理を実行します。
- 初期カメラ距離を少し遠めにし、`Reset View` / `A` キーで戻したときに地形全体だけでなく背景も入りやすいようにしました。

## 0.19.108 - 2026-05-26 14:57

- ボリューム雲の環境光色相を、天頂色だけでなく地平色も 40% 混ぜた色へ寄せ、地形の半球 ambient と近く見えるようにしました。
- 混ぜた色は従来の天頂色に近い輝度と彩度を保つよう補正し、明るさや彩度が変わりすぎないようにしました。

## 0.19.107 - 2026-05-26 13:37

- `Color Temperature (K)` を HDR 設定から独立した Camera タブの `Color` 設定へ移し、HDR ビューポートと SDR ビューポートの両方で使えるようにしました。

## 0.19.106 - 2026-05-26 13:19

- Camera タブの HDR 設定に `Color Temperature (K)` を追加し、HDR ビューポートの色温度を低い値で暖色、高い値で寒色へ調整できるようにしました。
- `HDR Viewport` がオフのときも露出と色温度の項目を非アクティブ状態で表示するようにしました。

## 0.19.105 - 2026-05-26 09:08

- `HDR Viewport` と露出設定を、表示設定タブから Camera タブへ移動しました。HDR と露出をカメラ / 撮影設定としてまとめて調整できます。

## 0.19.104 - 2026-05-26 08:52

- 表示設定に `HDR Viewport` を追加し、HDR ビューポートをオン / オフできるようにしました。オフにした場合は HDR 以前と同じ 8-bit ビューポート経路を使い、トーンマップと露出調整を通さずに表示します。

## 0.19.103 - 2026-05-26 08:35

- HDR ビューポートの最終表示にトーンマップ処理を追加し、表示設定から `Manual` / `Auto` の露出モードを切り替えられるようにしました。手動露出では `Exposure EV`、自動露出では `Bias EV` と露出範囲を調整できます。
- 自動露出が測光結果へ即座に切り替わらないよう、1x1 の露出履歴を使って時間をかけて追従するようにしました。表示設定に `Speed` を追加し、露出変化の速さを調整できます。

## 0.19.102 - 2026-05-26 08:18

- 3D ビューポートの内部カラーターゲットを 16-bit float HDR 形式に変更し、地形、天球、雲、水面、Depth of Field の描画を HDR バッファ上で処理するようにしました。

## 0.19.101 - 2026-05-26 07:57

- 地形プレビューの陰、シャドウマップ影、雲影の色味が揃うよう、影では直射光だけを減衰し、影の環境光は陰側と同じ horizon 寄りの色へブレンドする照明計算へ調整しました。

## 0.19.100 - 2026-05-26 07:45

- `Shape` ノードの `Shape Type` に `Box` を追加し、指定した Scale 全体を一定高さで埋める箱型ハイトフィールドを生成できるようにしました。

## 0.19.99 - 2026-05-26 06:56

- プロジェクトやアプリ設定の読み込み完了メッセージがステータスバーに残り続けないよう、数秒後に `Ready` へ戻る一時ステータスにしました。

## 0.19.98 - 2026-05-26 06:30

- エクスポート未実行時の `No export yet` 表示をなくし、エクスポート成功または失敗後だけステータスに結果を表示するようにしました。

## 0.19.97 - 2026-05-26 06:30

- UI 言語を切り替えても、主要タブの選択状態と設定パネルの開閉状態が維持されるよう、言語依存の表示ラベルから安定した内部 ID を分離しました。

## 0.19.96 - 2026-05-25 20:53

- Sun Mode が `Date Time` のときに、`Animate Time`、`Day Length (sec)`、`Skip Night` を追加しました。`Skip Night` は太陽高度が `-5°` を下回ると、次に `-5°` へ戻る時刻まで早送りします。

## 0.19.95 - 2026-05-25 20:24

- Sky / Clouds タブの太陽、影、大気散乱、ボリューム雲設定を英語 UI に対応し、英語表示で日本語ラベルや説明が混ざらないようにしました。

## 0.19.94 - 2026-05-25 20:13

- ビューポート左上の `表示` ポップアップから `カメラを回す` を外し、自動回転の操作場所を Camera タブに整理しました。

## 0.19.93 - 2026-05-25 20:08

- Camera タブのカメラ項目と Depth of Field 項目を英語 UI に対応し、英語表示で日本語ラベルや説明が混ざらないようにしました。

## 0.19.92 - 2026-05-25 20:04

- デモ用に、カメラを Y 軸まわりへ自動回転させる `カメラを回す` と `角速度 (deg/s)` を Camera タブへ追加しました。3D ビューポート左上の `表示` ポップアップからもオン / オフできます。

## 0.19.91 - 2026-05-25 18:55

- ノードの丸い出力ピンでは表示テクスチャーを切り替えず、出力タイトルのクリックでは従来どおり切り替えられるようにしました。

## 0.19.90 - 2026-05-25 18:52

- ノードの出力ピンや出力ラベルをクリックしても、3D ビューポートの表示テクスチャーが切り替わらないようにしました。

## 0.19.89 - 2026-05-25 14:34

- `Simulation Resolution` と `Viewport Mesh Resolution` のプリセットに `4096` を追加しました。

## 0.19.88 - 2026-05-25 14:25

- Path ノード編集中に `L + 左ドラッグ` で太陽方向を操作したとき、最初のクリックが Path ポイント追加として扱われないようにしました。

## 0.19.87 - 2026-05-25 14:21

- 3D ビューポート上の太陽方向ドラッグ操作を `Shift + Alt + 左ドラッグ` から `L + 左ドラッグ` へ変更しました。

## 0.19.86 - 2026-05-25 14:19

- 太陽方向ギズモの中心点を、輪郭のないシンプルな青いサークル表示へ調整しました。

## 0.19.85 - 2026-05-25 14:17

- 太陽方向ギズモの中心点を、方位リングに合わせた青系の表示へ調整しました。

## 0.19.84 - 2026-05-25 14:14

- 太陽方向ギズモの方位投影ラインを、中心から外円まで伸びる表示へ調整しました。

## 0.19.83 - 2026-05-25 14:10

- 太陽方向ギズモの太陽ベクトル矢印で、軸線が矢印ヘッドの先端まで突き抜けないようにしました。

## 0.19.82 - 2026-05-25 14:10

- 太陽方向ギズモの太陽ベクトル矢印を太陽方向側から中心へ向かう向きにし、中心手前で止まるようにしました。

## 0.19.81 - 2026-05-25 14:03

- 3D ビューポートで太陽方向をドラッグしている間、太陽ベクトルの矢印、水平リング、方位投影線、高度アークを一時表示するようにしました。

## 0.19.80 - 2026-05-25 13:53

- 3D ビューポート左上の `表示` ポップアップにあるチェックボックスの角丸を小さくし、角ばった見た目へ調整しました。

## 0.19.79 - 2026-05-25 11:57

- 英語 UI で日本語が残っていたノードプロパティの説明文、パス設定、共通の `Browse` / 既定値リセット表示を英語へ切り替えるようにしました。

## 0.19.78 - 2026-05-25 11:29

- アプリ設定に UI 言語を追加し、上部メニューの `設定` > `言語` と右ペインの `設定` タブから `English` / `日本語` を切り替えられるようにしました。
- 主要なメニュー、ビューポート / ノードネットワーク / 右ペインのタブ、設定パネル、3D ビューポート左上の `表示` ポップアップ、ノード未選択時の案内を、選択中の UI 言語に応じて切り替えるようにしました。

## 0.19.77 - 2026-05-25 11:21

- 3D ビューポート上で `Shift + Alt + 左ドラッグ` すると、太陽の方位角と高度を直接調整できるようにしました。
- `Sun Mode` が `Date Time` のときに太陽ドラッグを始めると、現在の計算済み太陽位置を保ったまま `Manual` へ切り替えるようにしました。

## 0.19.76 - 2026-05-25 11:14

- 3D ビューポート左上の `表示` ポップアップで、項目を切り替えてもポップアップを閉じず、外側をクリックしたときに閉じるようにしました。

## 0.19.75 - 2026-05-25 11:12

- `Path` ノードと `Path` 型ピン / リンクのアクセント色を、少し彩度を落とした水色へ調整しました。

## 0.19.74 - 2026-05-25 11:09

- `Path` ノードと `Path` 型ピン / リンクのアクセント色を、より水色寄りに調整しました。

## 0.19.73 - 2026-05-25 07:25

- `Mask Fluvial` の `Particles` モードを CPU 並列化しました。粒子を複数スレッドへ分配し、スレッドごとの一時 hit map を最後に合成することで書き込み競合を避けます。
...
- `Mask Path` などで作成したマスクから地形を生成できるように、ノード作成メニュー、プロパティ、保存/読み込み、プレビュー評価に対応しました。

## 0.19.67 - 2026-05-25 06:05

- Path のポイントに `Intensity` を追加しました。`Mask Path` では各ポイントの `Intensity` を 0..1 の強さとして使い、エッジ上では両端ポイントの値を補間します。
- Path のポイント `Intensity` はプロパティで編集でき、プロジェクト保存・読み込みにも対応しました。

## 0.19.66 - 2026-05-25 05:55

- 既存プロジェクトや既に配置済みのノードでも、旧表示名の `Path Mask` が残らず `Mask Path` と表示されるようにしました。
- `Mask Path` の CPU 評価を行単位で並列化し、ポイントとエッジ情報をピクセルループの外で事前展開するようにしました。高解像度プレビューやポイント数が増えた場合の再計算負荷を下げています。

## 0.19.65 - 2026-05-25 05:45

- `Mask Path` ノード名に統一し、マスク系ノードとして一覧で見たときに揃う名前にしました。
- Path のポイントごとの `Width` / `Feather` を編集した直後に `Mask Path` のプレビューへ反映されるようにしました。マスク生成では default ではなく各ポイントの値を優先し、エッジ上では両端ポイントの値を補間します。

## 0.19.64 - 2026-05-25 05:35

- `Mask Path` ノードを追加しました。`Path` 入力から `Mask` を生成し、各ポイントの `Width` / `Feather` をエッジ上で補間して中心線からの距離でマスク化します。
- `Mask Path` に `Gamma` と `Invert` を追加し、プロジェクト保存・読み込み、マスクプレビュー、Mask 系ノードへの接続に対応しました。

## 0.19.63 - 2026-05-25 05:12

- Path のポイントごとに `Width` と `Feather` を持てるようにし、選択ポイントのプロパティで編集できるようにしました。
- Path のエッジ分割で追加されるポイントは、両端ポイントの `Width` と `Feather` を補間して引き継ぐようにしました。

## 0.19.62 - 2026-05-25 05:00

- ノード追加メニューから生成したノードの配置位置が、マウスカーソル位置から大きくずれる問題を修正しました。

## 0.19.61 - 2026-05-25 04:44

- Path 編集モード中は `Ctrl` 操作で Camera の被写界深度フォーカスピックへ入らないようにしました。
- 被写界深度フォーカスピックのカーソル表示と `Ctrl+クリック` 操作は、`Depth of Field` が有効なときだけ発動するようにしました。

## 0.19.60 - 2026-05-25 04:35

- ノード追加メニューからノードを生成したとき、ノードをビュー中心ではなくメニューを開いたマウスカーソル位置へ配置するようにしました。

## 0.19.59 - 2026-05-25 04:27

- Path ノードの選択ポイントプロパティで、`Height Offset` 入力欄が親エリアいっぱいに伸びないようにしました。

## 0.19.58 - 2026-05-25 04:19

- Path 編集で通常のエッジクリックはエッジ選択にし、`Ctrl+エッジクリック` でエッジ上にポイントを挿入するようにしました。

## 0.19.57 - 2026-05-25 04:05

- Path ノードの選択中ポイント座標を、プロパティパネルで `Position (Vector3)` として編集できるようにしました。

## 0.19.56 - 2026-05-25 03:38

- Path ノードのプロパティに選択中ポイントの ID、X/Z 座標、高さ、高さオフセット、高さモードを表示し、プロパティ編集をビューポート表示へ反映できるようにしました。

## 0.19.55 - 2026-05-25 03:34

- Path 編集中にビューポート上の `表示` ボタンや表示ポップアップをクリックしても、ポイント追加や Path 選択操作として扱わないようにしました。

## 0.19.54 - 2026-05-25 03:18

- Path の移動ギズモから X/Y/Z ラベルを外しました。
- Path の移動ギズモ中心をドラッグしたとき、カメラ平面に沿ってポイントを移動できるようにしました。

## 0.19.53 - 2026-05-25 02:47

- Path 編集でポイント選択中に `W` キーを押すと移動ギズモを表示できるようにしました。X/Y/Z 軸の矢印をドラッグしてポイントを移動できます。

## 0.19.52 - 2026-05-25 02:36

- Path 編集でポイントをクリック選択できるようにしました。選択ポイントから次のクリックで続きを伸ばせます。
- Path 編集でエッジをクリックした位置に新しいポイントを挿入し、エッジを分割できるようにしました。
- Path 編集で選択中のポイントを `Del` キーで削除できるようにしました。ポイント削除時は接続しているエッジも削除されます。

## 0.19.51 - 2026-05-25 02:30

- 3D ビューポートと 2D マップビューのナビゲーション操作を `Alt` キー併用に統一しました。`Alt+左ドラッグ` で回転、`Alt+右ドラッグ` / `Alt+中ドラッグ` でパン、`Alt+ホイール` でズームします。

## 0.19.50 - 2026-05-24 23:11

- `Path` ノードを追加しました。Path ノード選択中は 2D/3D ビューでポイントを追加でき、連続したポイントは自動でエッジ接続されます。
- 3D ビューで地形を拾えない場合でも、Path ノードのポイントを XZ 平面に追加できるようにしました。
- Path 編集中に `Enter` キーで現在の連続線を確定し、次のクリックを新しい線の始点として扱えるようにしました。

## 0.19.49 - 2026-05-24 21:41

- `表示` メニューに `レイアウトを初期化` を追加し、ウィンドウサイズと画面分割幅を既定状態へ戻せるようにしました。

## 0.19.48 - 2026-05-24 19:46

- デバッグログの自動スクロールが、ログ本文欄の実際のスクロール領域へ直接反映されるように修正しました。

## 0.19.47 - 2026-05-24 19:43

- デバッグログの自動スクロールが、ログ本文欄のフォーカス状態に関係なく末尾へ追従するように修正しました。

## 0.19.46 - 2026-05-24 19:38

- 選択可能なデバッグログで、自動スクロールがログ本文欄の末尾へ確実に追従するように修正しました。

## 0.19.45 - 2026-05-24 19:38

- デバッグログの自動スクロールが、選択可能なログテキスト欄の内部スクロールにも効くように修正しました。ログテキストを選択している間は、自動スクロールで選択操作を邪魔しないようにしました。

## 0.19.44 - 2026-05-24 19:34

- デバッグログの本文を読み取り専用の複数行テキスト欄に変更し、ログテキストを選択してコピーできるようにしました。

## 0.19.43 - 2026-05-24 19:31

- デバッグログの操作行を整理し、`クリア` ボタンを右寄せにしてタブ直下で不自然に見えない配置にしました。

## 0.19.42 - 2026-05-24 19:25

- 下部ステータスバーの上端にも、ウィンドウ分割と同じ薄い区切りラインを表示するようにしました。

## 0.19.41 - 2026-05-24 12:22

- ビューポート右上の FPS、負荷、ドローコールのデバッグ表示を低彩度のグレー配色にし、緑がかった見え方を抑えました。

## 0.19.40 - 2026-05-24 11:57

- ウィンドウ分割ラインの見た目は 1px のまま、マウスの当たり判定だけを広げて掴みやすくしました。

## 0.19.39 - 2026-05-24 11:55

- ビューポート、デバッグ領域、ノード、プロパティなどのウィンドウ間にある分割部分を、幅のある帯ではなく 1px のラインとして表示するようにしました。

## 0.19.38 - 2026-05-24 05:57

- 雲設定の高度指定を `Altitude Min / Max` から `Altitude / Thickness` に変更しました。厚みを保ったまま雲全体の高度を上下できるようになります。
- `Colorize` の GPU compute 実装を `src/gpu/` 配下へ分離し、GPU compute backend 共通の D3D12 実行コンテキストを追加しました。

## 0.19.37 - 2026-05-24 05:40

- 3D ビューポート上で `Ctrl+クリック` して被写界深度のフォーカス距離を指定する操作が効かなくなっていた問題を修正しました。
- `Mask Noise` の GPU compute 実装を `src/gpu/` 配下へ分離し、今後ほかの GPU backend を分割しやすい構成にしました。

## 0.19.36 - 2026-05-24 03:22

- ノードヘッダーの山アイコンを調整しました。単純な三角形から、背面の山、稜線、雪筋、陰影を持つシルエットに変更し、小さい表示でも地形ノードらしく見えるようにしました。

## 0.19.35 - 2026-05-24 03:10

- 雲から雲への影を `Cloud-to-Cloud Shadows` チェックボックスでオン/オフできるようにしました。オフのときは太陽方向ライトマーチによる雲内自己遮蔽を止め、関連するサンプル数・ステップ幅・位相関数の設定を非アクティブ表示にします。オン時の効果が見えやすいよう、遮蔽されている部分では直射光に加えて雲内部の拡散光も少し落とすようにしました。

## 0.19.34 - 2026-05-24 02:59

- 雲の影側に空色の環境光を足す `Shadow Ambient` を追加しました。自己遮蔽が強い部分ほど天球の青みを受け、雲底や影側が冷たい色で持ち上がるようになります。

## 0.19.33 - 2026-05-24 02:48

- デバッグ領域のタイトル行を外し、閉じるボタンをタブバー側へ移して縦幅を抑えました。
- 上部メニューの表示項目名を、開く対象に合わせて `デバッグログ` から `デバッグ` へ変更しました。
- ウィンドウ分割の見た目を細いラインに寄せ、操作用の当たり判定は残すようにしました。
- デバッグ領域の閉じるボタンを小さめの `×` ボタンに調整しました。
- デバッグ領域の閉じるボタン内の `×` を中央に揃えるようにしました。
- ステータスバーのテキスト位置を少し上げ、縦中央に近づけました。

## 0.19.32 - 2026-05-24 01:49

- ビューポート下にデバッグログウィンドウを追加し、上部メニューの `表示` > `デバッグログ` から表示できるようにしました。
- デバッグログウィンドウは `×` ボタンで閉じられ、プロジェクト、エクスポート、評価、GPU 関連ステータスの変化を時刻付きで確認できます。
- 左右分割後の左カラム内だけでビューポートとデバッグログを上下分割し、右側のノード/プロパティ領域の高さ分割と連動しないようにしました。
- デバッグログを独自ヘッダーではなくタブ表示に変更しました。
- インスペクタの `Debug` タブをデバッグ領域へ移動しました。
- デバッグ領域のタブ内容に、プロパティ領域と同じ余白を付けました。
- デバッグ領域全体の閉じるボタンを追加し、タブ名を `ログ` と `診断` に整理しました。
- `Debug` タブに `Frame Timing` 表示チェックを追加し、オンのときは元の Frame Timing 項目をプレビュー上へオーバーレイ表示するようにしました。詳細な Frame Timing 一覧は `Debug` タブ内から外しました。
- Frame Timing オーバーレイの項目名は色分けせず、通常のオーバーレイ文字色で統一しました。
- Frame Timing オーバーレイから通常確認では必要性の低い `Window` 行を外しました。

## 0.19.31 - 2026-05-24 01:11

- `エクスポート` タブをノードテクスチャー出力用に更新し、現在プレビュー中の `Mask`、`Color Texture`、`Heightmap` を PNG として書き出せるようにしました。
- テクスチャー書き出しでは現在の `Simulation Resolution` を使い、出力解像度はその設定値以下のプリセットから選べるようにしました。
- 書き出し形式は `Mask` が 8bit モノクロ、`Color Texture` が 8bit カラー、`Heightmap` が 16bit モノクロです。
- `エクスポート` タブのUIを左寄せの行レイアウトにし、出力パス横のフォルダボタンから保存先フォルダを開けるようにしました。

## 0.19.30 - 2026-05-23 22:52

- 上部メニューから `表示` メニュー内の `Mesh` 切り替え項目を削除しました。

## 0.19.29 - 2026-05-23 22:48

- `Camera` タブの `Depth of Field` で、`有効` がオフのときも各パラメータを非アクティブ表示するようにしました。

## 0.19.28 - 2026-05-23 22:45

- 水面設定で `Water Surface` がオフのときも、水位、透明度、反射、屈折などのパラメータを非アクティブ表示するようにしました。

## 0.19.27 - 2026-05-23 22:43

- 2D ビューの描画サンプル上限を `1024 x 1024` に戻しました。デバッグ調査中に下げていた `256 x 256` 上限を解除し、高解像度のハイトマップ、マスク、カラーテクスチャを細かく確認できるようにします。

## 0.19.26 - 2026-05-23 16:45

- `Crumbling` ノードに `Spread (%)` を追加しました。岩屑が進行方向から横へ逸れる量を調整でき、停止後の岩片も簡易的に押し分けて、一直線に重なって見える結果を抑えます。

## 0.19.25 - 2026-05-23 12:22

- 水面断面側壁の三角形 winding を反転し、外向き法線と一致させました。水面の裏面カリング有効時に断面が内側を向いて扱われる問題を修正します。

## 0.19.24 - 2026-05-23 12:20

- 水面描画の PSO でも裏面カリングを有効にしました。水面上面と同じ winding の表側を残すため、地形サーフェスと同じく front face をカリングします。

## 0.19.23 - 2026-05-23 12:18

- 水面上面の裏向きピクセルをシェーダー側で破棄するようにしました。水メッシュ全体の両面描画は維持しつつ、断面側壁は表示したまま、上面の裏側だけを描かないようにします。

## 0.19.22 - 2026-05-23 12:07

- 水面設定に `Fresnel Power` を追加しました。既定値 5.0 は従来の Schlick 近似相当で、小さい値ほど高い角度から反射が入り、大きい値ほど低い角度だけで反射します。

## 0.19.21 - 2026-05-23 11:55

- 水面設定から `Refraction Blur` を削除しました。現在の弱い屈折では見た目への影響が小さいため、上面屈折は単一サンプルの小さな歪みに簡略化しました。旧プロジェクトに保存済みの `waterRefractionBlur` は読み込み時に無視されます。

## 0.19.20 - 2026-05-23 11:55

- 水面断面側壁の地形接触線フェードをなだらかにしました。接触線でアルファを急激に落とさず、最低でも本来の透明度の 68% を残すことで、境界が白く抜ける見た目を抑えます。

## 0.19.19 - 2026-05-23 11:48

- 3D ビューポートの地形サーフェス描画で裏面カリングを有効にしました。水面、断面壁、ワイヤーフレーム、グリッドは従来どおり両面/ライン描画として扱います。

## 0.19.18 - 2026-05-23 11:41

- 水面シェーダーを自然でシンプルな方向へ整理しました。上面は Beer-Lambert 型の透過率を中心にし、屈折は浅瀬の弱い補助だけに抑えました。
- 水面断面側壁のスクリーンスペース屈折を廃止し、水柱の厚みによる色と透明度だけで表示するようにしました。
- 水面上面の強制的な最低 α (`0.40`) を廃止し、水深、濁り、フレネル、スペキュラーから透明度が決まるようにしました。
- `Refraction Strength` と `Refraction Blur` の既定値を控えめにしました。

## 0.19.17 - 2026-05-23 11:31

- 水面設定に `Screen Space Reflections` チェックボックスを追加しました。画面内の地形を水面へ映す SSR は自然さを優先して既定オフにし、`Reflection Strength` は空反射を含む反射全体の強さとして残しました。

## 0.19.16 - 2026-05-23 11:21

- 水面のスクリーンスペース屈折で画面外や画面端を参照したとき、端のピクセルが引き伸ばされて地形が水面内へはみ出して見える問題を修正しました。画面外サンプルは屈折寄与を弱め、水本来の色へ自然に戻します。
- 屈折サンプルの深度と復元ワールド座標を検査し、水面より手前にある地形や水位より上の地形を屈折色として使わないようにしました。

## 0.19.15 - 2026-05-23

- 断面側壁の屈折ウォブルをアニメーションから切り離しました。アニメーションが有効な場合でも断面の歪みは静止したままになります。

## 0.19.14 - 2026-05-23

- 水面スペキュラーのセルベースきらめき (`sparkleGlint`) を廃止しました。`floor()` によるハードエッジのセルが四角いドットとして見えていました。
- 代わりに `specularGradient` にバイリニア補間つきのバリューノイズ (高周波、スケール 4% / 2.5%) を加えるようにしました。specularNormal が不規則に揺らぐことで自然な散乱きらめきが得られ、セル境界が見えません。
- `sharpSpec` / `glintSpec` の 2 段構成できらめきを作るようにしました。

## 0.19.13 - 2026-05-23

- 水面スペキュラーのグリッドパターン (デジタル迷彩状) を修正しました。原因はきらめきセルが全面で常に異なる明るさを持つことでした。ガウシアン glitter path のシグマを 22% → 7% に縮小し、スパークルセルを全体の約 18% に限定しさらに `pow(..., 14)` で点灯時間を極短くすることで、ほぼ 0 の暗い状態が基準になりごく一部のセルだけが瞬間的に輝くようになりました。

## 0.19.12 - 2026-05-23

- 水面法線のノイズ関数をバイリニア補間つきのバリューノイズ (`WaterValueNoise`) に置き換えました。従来の `floor()` ベースのセルハッシュはグリッドセル境界で値が不連続になりドット状のパターンが出ていました。新関数は隣接 4 セルの値を smoothstep 補間してなめらかなノイズを生成します。

## 0.19.11 - 2026-05-23

- 水面上面の太陽スペキュラーを再設計しました。従来の `dot(reflect(-L, flat), V)` ベースのパスマスクはカメラ方向がほぼ均一なため水面全体が一様に白くなる問題がありました。新方式では「カメラ位置から平坦水面に太陽光が鏡反射する正反射点」をワールド座標で計算し、各ピクセルからその点への距離で楕円ガウシアン形状のパスを生成します。太陽の水平方向に沿って縦長・細い帯状の反射が得られ、パス外の水面は暗くなります。
- スペキュラーの総輝度に上限 (1.8) を設け、白飛びを防ぎました。

## 0.19.10 - 2026-05-23

- 水面上面の描画を再設計しました。屈折した地形色が水体色を上回り水面全体が一様に明るくなっていた問題を修正しました。
  - 屈折 UV オフセットを縮小 (最大 ~0.036 → ~0.011) して波法線の周期パターンがチェック柄として現れなくなりました。
  - `underwaterView` の地形貢献を最大 30% に制限し、水体色が主体になりました。
  - αを吸収量ベースに戻しました (`waterRefractionStrength * 0.72` の一律フロアを廃止)。
- 太陽グリッターを再設計しました。`sunGlintPath = smoothstep(0.82, 0.998)` で太陽方向付近にのみ集中させ、広域グロー + 中域輝き + 白い鋭い輝点 + タイムベースの散乱きらめき点の 4 層構成にしました。パス外の水面は暗いままになり、参照画像のような太陽の光路が得られます。

## 0.19.9 - 2026-05-23

- 水面上面の岸際に明るいリングが出る問題を修正しました。屈折 UV オフセットを岸際 (`shallowDepth`) に比例してスケールダウンし、浅瀬では波の歪みを抑えます。また屈折先の地形が水位より高い (水面外) 場合はオフセットなし UV にフォールバックするバリデーションを追加しました。
- 断面側壁の底辺 (地形接触線) が明るく見える問題を修正しました。`contactFade` が 0 になっても最低α (0.25 × `baseAlpha`) を残し、背後の地形断面色が完全に透けないようにしました。
- 断面側壁の上端 (水面との接合部) を薄くフェードするようにしました。水面上面との境界線が和らぎます。

## 0.19.8 - 2026-05-23

- 断面側壁の屈折描画を上面と同じ設計に統一しました。従来は吸収αを「ピクセルの縦位置 (`distanceFromTerrain`)」で計算していたため底部のαが低くコンポジッターに上書きされていました。新方式では「横方向の水柱厚さ (`waterColumnDepth`)」で吸収を計算し、`waterRefractionStrength * 0.72` を最低αとして確保することで、屈折・ブラー効果が実際に画面に出るようになります。
- 断面の色グラデーションは引き続き縦位置 (`distanceFromTerrain`) で制御します (底 = 深色、上 = 浅色)。
- 断面のブラー半径を吸収量に連動させるようにしました。水が薄い (浅瀬) ではブラーが小さく、水が深い (厚い水柱) では広がります。

## 0.19.7 - 2026-05-23

- 水面上面の屈折描画を根本から再設計しました。従来は「水のRGBに屈折を色として乗せてαブレンドする」方式で、透明な水のαが低いためにコンポジッターが素の地形 (非屈折) で上書きしていました。新方式では「屈折した地形色をRGBに焼き込んで、αを吸収量 + 屈折強度ベースで十分に確保する」ようにし、透明な浅い水でも屈折・ブラー効果が実際に見えるようになります。
- 屈折強度 (`Refraction Strength`) が 0 のとき α も 0 に寄り、1 のとき α が最低 0.72 確保されます。これにより波の歪みと水深ブラーが常に画面に出ます。
- 岸際のフェードは維持されます。

## 0.19.6 - 2026-05-23

- 水面上面の屈折オフセットを拡大しました (最大 ~0.012 → ~0.036)。波の法線によるディスプレイスメントが画面上でより見えるようになります。
- 水面上面の屈折ビューにブラーを追加しました。水深が深いほど水中の地形がにじんで見え、浅い部分ではシャープに透けて見えます。ブラー量は `Refraction Blur` で調整できます。
- 断面側壁の歪みオフセット半径とブラー半径を拡大しました (最大ブラー ~0.041 → ~0.063)。
- 断面の水色ティントの乗せ方を変更しました。ブラーした背景に薄く水色を乗せてから濃さで制御するようにし、薄い水は背後の地形がぼけて透け、厚い水は水色が支配的になる段階的な変化にしました。
- 断面の屈折に `waterRefractionStrength` が直接かかるようにし、0 で屈折なし・1 が標準の効き方になりました。
- 断面の水色に大気の地平線色と太陽色を少量ミックスし、上面と色調が揃うようにしました。

## 0.19.5 - 2026-05-23

- 波アニメーションのオン / オフを切り替える `Animation` チェックボックスを追加しました。オフにすると波形が静止します。
- 水面への地形・空の映り込み強度を調整する `Reflection Strength` スライダーを追加しました。SSR による実際の地形色の映り込みにも影響します。
- 断面側壁のにじみ量を調整する `Refraction Blur` スライダーの範囲を 0〜1 から 0〜2 に広げました。
- 断面側壁の屈折ブラー半径を拡大し、薄い水では背後の地形色が透け、厚い水では水色へ戻る見た目をより明確にしました。断面のぼかしは深さに応じてグラデーション状に変化します。
- 断面側壁のアニメーション (揺らぎ) もアニメーションフラグに連動するようにしました。
- 反射スペキュラーのブレンド式を改善し、低仰角カメラでの地形の映り込みを強くしました。
- パラメータパネルを「波」「反射」「屈折 / 断面」の 3 セクションに整理しました。

## 0.19.4 - 2026-05-23

- 水面波形にアニメーションを追加しました。各波に深水分散関係 (`ω = √(g·k)`) に基づく時間位相を与え、静止した正弦波が作っていた規則的な縞模様をなくしました。
- 波法線に小スケールのハッシュノイズを重ね、アニメーション後に残る周期的なパターンをさらに崩すようにしました。
- 水面の浅瀬色・深瀬色に空の地平線色と太陽色を少量ミックスするようにしました。空の時刻・天候が変わったときに水面の色調も連動します。
- 水面反射の地形色を、カラーテクスチャが有効な場合は実際の地形色 (colorTexture) から視差UVでサンプルするようにしました。従来の高さベースのグレー近似より地形の色味が反射に正しく出ます。
- 空反射の地平線付近に太陽色を薄く加え、夕日など低仰角の太陽時に空の色合いが水面に馴染むようにしました。

## 0.19.3 - 2026-05-23 08:08

- `Water Color` のデフォルトを `0.05, 0.34, 0.58` から `0.04, 0.24, 0.36` へ変更し、反射や屈折が乗った状態で自然に見える低彩度の青緑にしました。
- 水面上面にスクリーンスペース・リフレクション (SSR) を追加しました。水面描画前のシーン深度とシーン色をコピーし、反射方向へ短くレイマーチして、画面内の地形色を水面へ薄く映り込ませます。ヒットしない場所や画面外は従来の空反射と高さベースの地形近似へフォールバックします。
- 水面上面にスクリーンスペース・リフラクションを追加しました。水面描画前のシーン色をコピーし、水面法線で少し歪ませてサンプルすることで、透明な水越しの地形が屈折して見えるようにしました。
- 水面断面にも弱いスクリーンスペース・リフラクションを追加しました。側壁では軽いブラーを混ぜ、厚い水では水色へ戻すことで、水っぽさを出しつつ過度にガラス板のように見えないようにしました。
- 水面設定に `Refraction Strength` と `Refraction Blur` を追加しました。`Refractive Index` はフレネル反射用として残し、スクリーンスペース屈折の歪み量と断面のにじみ量を別に調整できるようにしました。
- 断面側壁の `Refraction Blur` が見た目に分かりやすく効くように、ブラー半径とサンプル数を増やしました。
- `Water Level (m)` の下限を 0 m に変更しました。既存設定やプロジェクトで負の水位が保存されている場合は読み込み時に 0 m へ丸めます。
- プロジェクト読み込み直後のノード位置復元をノード描画前に行うようにし、ノードネットワークが一瞬だけ初期位置で重なって見える問題を修正しました。

## 0.19.2 - 2026-05-23 07:30

- 水面の `Opacity` を最終アルファ上限ではなく、水の濁り / 吸収距離として扱うようにしました。`0` では遠くまで透き通り、`1` では短い距離で不透明になります。
- `Opacity` が `0` の場合も水面メッシュを描画し、フレネル反射や太陽スペキュラーは残るようにしました。
- 水面法線を複数方向・複数波長の波の勾配合成に変更し、`Waves Scale (m)` の既定値を 24 m に調整しました。細かい格子状のハイライトを抑え、地形スケールに馴染む波模様にします。
- 水面上面の色と透明度を、鉛直水深ではなくカメラ方向の水中距離を主に使って計算するようにしました。斜め視点で真下の地形だけに引っ張られる見え方を抑えます。
- 水面スペキュラー用の法線を低周波寄りに分け、ハイライトの指数と強さを抑えて、白いノイズ状の反射が出にくい柔らかい見た目にしました。
- 太陽スペキュラーを平面水面の反射方向でマスクし、水面全体に散る反射ではなく、太陽方向にまとまった光の帯として出るようにしました。
- 太陽スペキュラーの反射帯を狭め、広い白ベタにならないよう強度を抑制しました。

## 0.19.1 - 2026-05-23 06:42

- 水面上面の透明度判定を、直下の水深だけではなくカメラ方向の短いハイトフィールドレイマーチで計算するようにしました。斜め視点で直下の隆起だけを拾って不自然に透ける問題を抑えます。
- 水面端で太陽スペキュラーが欠けて見える問題を修正しました。外周の反射フェードは地形近似反射だけに適用し、太陽スペキュラーは水面全体で維持します。
- 水面上面と断面側壁の透明描画順を調整し、外周側壁が上面のスペキュラーを暗く上書きしないようにしました。
- 水面上面を 4 頂点の一枚板ではなく、ハイトフィールドでクリップしたメッシュとして生成するようにしました。カメラ角度によって岸線が地形へ埋まって見える問題を抑えます。
- 水面とハイトフィールドの境界が完全に透明になって隙間のように見える問題を修正しました。浅瀬や断面下端は透明寄りに保ちつつ、岸際と接触線には控えめな水色のアルファを残します。
- 水面上面の透明度を鉛直水深ベースに寄せ、斜め視点で透明度がすぐ飽和して全体が同じ濃さに見える問題を抑えました。
- 水面断面はハイトフィールドとの差が 100 m 以上ある場所を完全に不透明にし、深い断面でも地形が透けて見える問題を抑えました。
- 水面上面もハイトフィールドとの差が 100 m 以上ある場所を完全に不透明にするようにしました。

- 水面表示を GeoGen 風に刷新しました。
  - 水位が 0 m より高い場合、地形ボックスの 4 辺に断面側壁を描画します。側壁は水の厚みが薄い場所ほど透明で、厚みのある場所ほど不透明に変化します。
  - 断面側壁の下辺がハイトフィールドの地形輪郭に沿うようにしました。地形が評価されるたびに壁メッシュを再生成します。
  - 上面の透明度をカメラ方向の光路長ベースに変更しました。`pathLength = waterDepth / cos(仰角)` で計算するため、斜め方向から見た水はより不透明になり、浅瀬・直上視点では透明になります。
  - 水面上面と断面側壁の透明度を、ハイトフィールドとの距離と水の厚みに基づいて調整しました。地形に近い浅瀬や断面下端はより透明になり、厚みのある場所ほど不透明になります。
  - `Waves Scale (m)` パラメータを追加しました。波の周期スケールを調整できます。
  - `Refractive Index` パラメータを追加しました。Schlick 近似でフレネル反射量を物理的に決定します（デフォルト 1.33、水相当）。
  - 反射は天空色 (地平線/天頂補間) と高さベースの地形色近似をフレネルでブレンドします。

## 0.18.62 - 2026-05-23 05:33

- `雲` タブで `有効` がオフのときも雲パラメータを表示し、非アクティブ状態としてグレーアウトするようにしました。

## 0.18.61 - 2026-05-23 05:25

- 右ペインのタブ構成を整理し、`天球` から雲設定を分離して独立した `雲` タブにしました。
- `水面` タブを追加し、`Water Surface`、`Water Level (m)`、`Opacity`、`Water Color` を表示設定タブから移動しました。
- 水面表示を水平な上面だけに変更し、地形の断面ポリゴンへ水の側面が重ならないようにしました。
- 水面シェーダーの太陽スペキュラー反射を強め、波による細かな反射の揺らぎを追加しました。

## 0.18.60 - 2026-05-23 05:10

- 表示設定に `Water Surface` を追加しました。`Water Level (m)` で水面の高さを調整でき、透明な水平面と外周の水ボリューム側面で湖や海面をプレビューできます。
- 水面の `Opacity` と `Water Color` を調整できるようにし、設定はプロジェクトファイルとアプリ設定へ保存されます。表示専用のため、ノード評価や OBJ エクスポートには影響しません。

## 0.18.59 - 2026-05-23 04:48

- ノード上の `Processing...` / `Pending` バッジを削除し、評価中ノードの表示は外枠パルスに集約しました。ステータスバーの状態表示は維持しています。

## 0.18.58 - 2026-05-23 04:40

- `Ctrl` 押下中にビューポートへ表示されるカメラフォーカス指定カーソルとヒット地点表示を、四隅のフォーカス枠と中央の小さな四角で構成したアイコンに変更しました。

## 0.18.57 - 2026-05-23 04:04

- `Processing...` / `Pending` のノードに、状態が見つけやすくなるよう前面レイヤーの外枠パルス表示を追加しました。処理中は緑系の枠とバッジで表示し、ズームに合わせて枠の角丸も変わるようにしました。
