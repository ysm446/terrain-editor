...
- 新しい UI 文言を追加するときは、英語 / 日本語の言語切り替えを意識する。内部 ID、保存形式、JSON キー、ログ互換性に関わる文字列は安定した英語のままにし、ユーザーに見えるメニュー、タブ、ラベル、ツールチップは翻訳テーブルや既存のローカライズ仕組みに載せる。
- English UI では、ユーザーに見える文字列に日本語を混ぜない。未翻訳の日本語が出ないよう、追加・変更した UI 範囲は英語表示を一通り確認する。
- 日本語 UI では専門用語を無理に和訳しない。`Heightmap`、`Mask`、`Path` など、制作ツール上で英語のままの方が自然な語は残してよい。操作説明や文章は自然な日本語にする。
